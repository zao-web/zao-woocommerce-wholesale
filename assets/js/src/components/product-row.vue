	...
	<tr :class="rowClass" :title="noStockTitle">
		...
		<td class="qty">
			<template v-if="hasStock">
				<input :tabindex="index + 1" size="3" @input="updateQty" :id="idAttr" :name="qtyName" :disabled="isDisabled" :value="qty" type="number" step="1" min="0" pattern="[0-9]"/><template v-if="minStock">&nbsp;<span style="inline-block">of {{ minStock }}</span></template>
			</template>
			<template v-else>
